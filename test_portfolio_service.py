...
os.environ['GEMINI_API_KEY'] = os.getenv('GEMINI_API_KEY', 'AIzaSyDvBEncIb21s9aIw396hVCvwUQoJOzZK7U')
...
def test_portfolio_service():
    ...
    print("2️⃣ Testing project description generation (fresh)...")
    ...
    print("3️⃣ Testing cached portfolio description...")
    with patch('httpx.AsyncClient') as MockAsyncClient:
        mock_client = AsyncMock()
        mock_client.get = mock_service_get
        MockAsyncClient.return_value.__aenter__.return_value = mock_client
        ...
        if data.get('cached'):
            ...
            print("   ⚠️  Cache not hit (might be timing)")
        print()
    
    # 4. Test get portfolio by repo_id
    print("4️⃣ Testing get portfolio by repo_id...")
    ...
    if response.status_code == 200:
        ...
        print(f"   ⚠️  No portfolio found for repo_id 2 (expected on first run)\n")
    else:
        print(f"   Status: {response.status_code}\n")
